  ...
  return (
    ...
    >
      ...
      <Box direction="Column" gap="100">
        <Text size="L400">{t('Pages.TokenLogin.title')}</Text>
        <Text size="T300">
          <b>{message}</b>
        </Text>
      </Box>
      ...
      {loginState.status === AsyncStatus.Error && (
        <>
          {loginState.error.errcode === LoginError.Forbidden && (
            <LoginTokenError message={t('Pages.TokenLogin.invalid_login_token')} />
          )}
          {loginState.error.errcode === LoginError.UserDeactivated && (
            <LoginTokenError message={t('Pages.TokenLogin.account_deactivated')} />
          )}
          {loginState.error.errcode === LoginError.InvalidRequest && (
            <LoginTokenError message={t('Pages.TokenLogin.invalid_request')} />
          )}
          {loginState.error.errcode === LoginError.RateLimited && (
            <LoginTokenError message={t('Pages.TokenLogin.rate_limited')} />
          )}
          {loginState.error.errcode === LoginError.Unknown && (
            <LoginTokenError message={t('Pages.TokenLogin.unknown_failure')} />
          )}
        </>
      )}
