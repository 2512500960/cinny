  ...
  return (
    <Box direction="Column" gap="400">
      ...
      <Button variant="Secondary" fill="Soft" onClick={onClose}>
        <Text size="B400">{t('Pages.DeviceVerification.close')}</Text>
      </Button>
    </Box>
  );
}
...
function VerificationWaitAccept() {
  ...
  return (
    <Box direction="Column" gap="400">
      ...
      <WaitingMessage message={t('Pages.DeviceVerification.waiting_accept')} />
    </Box>
  );
}
...
function VerificationAccept({ onAccept }: VerificationAcceptProps) {
  ...
  return (
    <Box direction="Column" gap="400">
      <Text>{t('Pages.DeviceVerification.accept_instruction')}</Text>
      <Button
        variant="Primary"
        fill="Solid"
        onClick={accept}
        ...
        disabled={accepting}
      >
        <Text size="B400">{t('Pages.DeviceVerification.accept')}</Text>
      </Button>
    </Box>
  );
}
...
function VerificationWaitStart() {
  ...
  return (
    <Box direction="Column" gap="400">
      ...
      <WaitingMessage message={t('Pages.DeviceVerification.waiting_response')} />
    </Box>
  );
}
...
function AutoVerificationStart({ onStart }: VerificationStartProps) {
  ...
  return (
    <Box direction="Column" gap="400">
      <WaitingMessage message={t('Pages.DeviceVerification.starting_verification')} />
    </Box>
  );
}
...
function CompareEmoji({ sasData }: { sasData: ShowSasCallbacks }) {
  ...
  return (
    <Box direction="Column" gap="400">
      <Text>{t('Pages.DeviceVerification.confirm_emoji_instruction')}</Text>
      <Box
      ...
      <Box direction="Column" gap="200">
        ...
        >
          <Text size="B400">{t('Pages.DeviceVerification.they_match')}</Text>
        </Button>
        <Button
          variant="Primary"
          fill="Soft"
          ...
          disabled={confirming}
        >
          <Text size="B400">{t('Pages.DeviceVerification.do_not_match')}</Text>
        </Button>
      </Box>
    </Box>
  );
...
function VerificationDone({ onExit }: VerificationDoneProps) {
  ...
  return (
    <Box direction="Column" gap="400">
      <div>
        <Text>{t('Pages.DeviceVerification.device_verified')}</Text>
      </div>
      <Button variant="Primary" fill="Solid" onClick={onExit}>
        <Text size="B400">{t('Pages.DeviceVerification.okay')}</Text>
      </Button>
    </Box>
  );
}
...
function VerificationCanceled({ onClose }: VerificationCanceledProps) {
  ...
  return (
    <Box direction="Column" gap="400">
      <Text>{t('Pages.DeviceVerification.verification_canceled')}</Text>
      <Button variant="Secondary" fill="Soft" onClick={onClose}>
        <Text size="B400">{t('Pages.DeviceVerification.close')}</Text>
      </Button>
    </Box>
        ...
        >
          <Dialog variant="Surface">
            <Header style={DialogHeaderStyles} variant="Surface" size="500">
              <Box grow="Yes">
                <Text size="H4">{t('Pages.DeviceVerification.title')}</Text>
              </Box>
              ...
              {phase === VerificationPhase.Started &&
                (request.verifier ? (
                  ...
                  <VerificationUnexpected
                    message={t('Pages.DeviceVerification.unexpected_error')}
