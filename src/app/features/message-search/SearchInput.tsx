...
export function SearchInput({ active, loading, searchInputRef, onSearch, onReset }: SearchProps) {
  ...
  return (
    <Box as="form" direction="Column" gap="100" onSubmit={handleSearchSubmit}>
      ...
      <Text size="L400">{t('Pages.SearchInput.search')}</Text>
      <Input
        ref={searchInputRef}
        ...
        size="500"
        variant="Background"
        placeholder={t('Pages.SearchInput.search_placeholder')}
        autoComplete="off"
        before={
          active && loading ? (
        ...
        after={
          active ? (
            <Chip
              ...
              onClick={onReset}
            >
              <Text size="B300">{t('Pages.SearchInput.clear')}</Text>
            </Chip>
          ) : (
            <Chip type="submit" variant="Primary" size="400" radii="Pill" outlined>
              <Text size="B300">{t('Pages.SearchInput.enter')}</Text>
            </Chip>
          )
        }
      />
