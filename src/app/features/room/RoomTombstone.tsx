  ...
  return (
    <RoomInputPlaceholder alignItems="Center" gap="600" className={css.RoomTombstone}>
      <Box direction="Column" grow="Yes">
        <Text size="T400">{body || t('Pages.RoomTombstone.replaced_no_longer_active')}</Text>
        {joinState.status === AsyncStatus.Error && (
          <Text style={{ color: color.Critical.Main }} size="T200">
            {(joinState.error as any)?.message ?? t('Pages.RoomTombstone.failed_join_replacement')}
          </Text>
        )}
      </Box>
      <Box shrink="No">
        ...
        joinState.status === AsyncStatus.Success ? (
          <Button onClick={handleOpen} size="300" variant="Success" fill="Solid" radii="300">
            <Text size="B300">{t('Pages.RoomTombstone.open_new_room')}</Text>
          </Button>
        ) : (
          <Button
            onClick={handleJoin}
          ...
          >
            <Text size="B300">{t('Pages.RoomTombstone.join_new_room')}</Text>
          </Button>
        )}
      </Box>
